...
// ─── Confirmed Asset Ref Panel ──────────────────────────
const ASSET_CAT_CONFIG: Record<string, { label: string; icon: any; color: string; bg: string }> = {
  character: { label: '캐릭터', icon: User, color: '#DB2777', bg: '#FCE7F3' },
  background: { label: '배경', icon: Image, color: '#0369A1', bg: '#E0F2FE' },
  sound: { label: '사운드', icon: Volume2, color: '#9333EA', bg: '#F3E8FF' },
  prop: { label: '소품', icon: Paintbrush, color: '#EA580C', bg: '#FFF7ED' },
  effect: { label: '이펙트', icon: Sparkles, color: '#059669', bg: '#ECFDF5' },
}

function ConfirmedAssetRefPanel({ confirmedAssets, projectId }: { confirmedAssets: ConfirmedAsset[]; projectId: string }) {
  const [open, setOpen] = useState(false)

  const grouped = confirmedAssets.reduce((acc, a) => {
    if (!acc[a.category]) acc[a.category] = []
    acc[a.category].push(a)
    return acc
  }, {} as Record<string, ConfirmedAsset[]>)

  const total = confirmedAssets.length

  return (
    <div className="rounded-xl border overflow-hidden" style={{ borderColor: '#10B981', background: '#F0FDF4' }}>
      <button
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {total > 0 ? (
            <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: '#10B981', color: 'white' }}>
              {total}개
            </span>
          ) : (
            <span className="text-[10px]" style={{ color: '#6EE7B7' }}>등록된 에셋 없음</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {total > 0 && (
            <span className="text-[10px]" style={{ color: '#059669' }}>에이전트 실행 시 자동 반영됨</span>
          )}
          {open ? <ChevronUp className="w-3.5 h-3.5" style={{ color: '#059669' }} /> : <ChevronDown className="w-3.5 h-3.5" style={{ color: '#059669' }} />}
        </div>
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
            <div className="px-4 pb-3 pt-2 border-t space-y-3" style={{ borderColor: '#A7F3D0' }}>
              {total === 0 ? (
                <div className="flex items-center gap-2 py-2">
                  <Lock className="w-4 h-4" style={{ color: '#6EE7B7' }} />
                  <div>
                    <p className="text-xs" style={{ color: '#065F46' }}>확정된 에셋이 없습니다.</p>
                    <p className="text-[10px] mt-0.5" style={{ color: '#059669' }}>
                      확정 에셋 페이지에서 캐릭터, 배경, 소품 등을 등록하면 에이전트가 자동으로 참조합니다.
                    </p>
                  </div>
                </div>
              ) : (
                Object.entries(grouped).map(([cat, assets]) => {
                  const cfg = ASSET_CAT_CONFIG[cat]
                  if (!cfg) return null
                  return (
                    <div key={cat}>
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        <span className="text-[11px] font-semibold" style={{ color: '#065F46' }}>{cfg.label} ({assets.length})</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5 ml-6">
                        {assets.map(a => (
                          <div key={a.id} className="flex items-center gap-1.5 px-2 py-1 rounded-lg border" style={{ background: 'white', borderColor: '#A7F3D0' }}>
                            {a.thumbnailUrl ? (
                              <img src={a.thumbnailUrl} alt={a.name} className="w-5 h-5 rounded object-cover shrink-0" />
                            ) : (
                            ...
                            <div>
                              <p className="text-[11px] font-medium leading-none" style={{ color: '#065F46' }}>{a.name}</p>
                              {a.prompt && (
                                <p className="text-[10px] mt-0.5 max-w-48 truncate" style={{ color: '#059669' }}>{a.prompt}</p>
                              )}
                            </div>
                            {a.prompt && (
...
export function SceneAIPanel({ scene, project, characters, projectId, episodeId, sceneId }: Props) {
  ...
  const { data: confirmedAssets = [] } = useQuery<ConfirmedAsset[]>({
  ...
  useEffect(() => {
    if (agentStore.status === 'done' && (agentStore.directorScript || agentStore.imagePrompts)) {
      setResult(prev => ({
        ...
        videoPromptCuts: agentStore.videoPromptCuts || prev?.videoPromptCuts || [],
  ...
  async function runAgent() {
    ...
    try {
      ...
      const res = await fetch('/api/agent/run', {
        ...
        body: JSON.stringify({ scene, project, characters, confirmedAssets }),
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {/* ─── 확정 에셋 참조 패널 ─── */}
        <ConfirmedAssetRefPanel confirmedAssets={confirmedAssets} projectId={projectId} />
